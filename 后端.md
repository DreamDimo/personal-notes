...
我们使用下面的方法进行本地请求的时候，发现接收方一直都没有收到请求（实际接收方已经在运行，所以肯定不是接受方的问题），经过调研是因为httpx走了代理端口，所以本地测试的时候要加上 `trust_env = False` 来保证不走代理，或者通过自己写 transport 定义连接来绕过代理 
...
async with httpx.AsyncClient(
    ...
    response = await client.post(
        # f"{conf.async_pipeline}/api/v1/tasks/llm",
        f"http://localhost:8001/api/v1/tasks/llm",
        json=payload
    )
```

1. 正常的路径（无 transport / 无代理）

你的代码请求 http://localhost:8001。

- **DNS 解析**：系统知道 localhost = 127.0.0.1。
- **数据包走向**：数据包直接在你的网卡内部“掉头”（Loopback），从客户端端口流向了 8001 端口。
- **结果**：服务收到请求。

2. 走代理的路径（有环境变量注入）

你的代码请求 http://localhost:8001，但 httpx 发现有代理 10.23.0.1。

- **封装请求**：httpx 不会去解析 localhost，而是把整个请求打包，发给 10.23.0.1。
- **发出的指令**：httpx 对代理服务器说：“帮我访问 http://localhost:8001”。

**这时候问题就来了：**

- **问题一：网络不可达（最可能的情况）**
  10.23.0.1 是你学校的内网 IP。如果你现在不在学校，或者没连校园网，你的电脑根本找不到 10.23.0.1 在哪里。**现象**：请求发出去后，石沉大海，直到超时（Timeout）。
- **问题二：语义歧义（即使网络通了）**
  假设你就在学校，网络也通了。代理服务器收到了你的请求：“请访问 localhost:8001”。在计算机网络里，localhost 是一个**相对概念**，永远指向**“我自己”**。代理服务器会尝试访问**它自己**的 8001 端口。显然，你的 async_pipeline 程序是运行在你的电脑上的，而不是运行在学校的代理服务器上的。**现象**：代理服务器告诉你的客户端：“我连不上 8001” 或者 “拒绝连接”。
...
dag = DAG(
    'branching_example',
    default_args=default_args,
    description='DAG with branching and parallel tasks',
    schedule_interval='@daily',
    start_date=datetime(2024, 1, 1),
    catchup=False,
    tags=['example']
...
# 设置依赖关系：branch -> [task_a, task_b, task_c] -> merge
...
task = PythonOperator(
    ...
    retry_exponential_backoff=True,  # 启用指数退避策略
    max_retry_delay=timedelta(hours=1),  # 最大重试延迟时间
    dag=dag
)
```
...
produce_task = PythonOperator(
    ...
    outlets=[raw_data],  # 标记该任务产生的数据集
    python_callable=produce_function,
    dag=dag
)

# Consumer 任务：消费原始数据并处理
consume_task = PythonOperator(
    task_id='consume_data',
    inlets=[raw_data],  # 依赖的数据集
    outlets=[processed_data],  # 处理后的数据集
...
def generate_tasks():
    ...
    for config in configs:
        task = PythonOperator(
            task_id=f"process_{config['table']}",  # 动态生成任务ID
            python_callable=process_table,  # 处理表的函数
            op_kwargs=config,  # 将配置传递给 python_callable
            dag=dag  # 绑定到当前DAG
        )
        tasks.append(task)
...
with TaskGroup('etl_group') as etl_group:
    ...
    # 设置任务依赖关系：extract -> transform -> load
    extract_task >> transform_task >> load_task

# 将 TaskGroup 作为任务添加到 DAG 中，并定义 DAG 任务顺序
