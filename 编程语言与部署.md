...
拉过来代码后，我们可以通过查看差异，git pull 会把旧的 head 储存在 `ORIG_HEAD` ，通过它可以查看变更内容
...
我们在 `crawler/walgreens` 开发，想要移动到 `develop_hy_langgraph `

#### 2.  操作步骤

```bash
git commit -m 'feat(hy): step7 扩展深度 random_walk_in 开发中...'
git checkout develop_hy_langgraph
git cherry-pick 684a46e3c6144be95c6fb5564886fe731a05c382 // 将walgreens的提交复制到develop_hy_langgraph
git checkout crawler/walgreens
git rebase develop_hy_langgraph // 将 walgreens 接到 develop_hy_langgraph
git checkout develop_hy_langgraph
git fetch
git pull --rebase origin develop_hy_langgraph
git push origin develop_hy_langgraph
git checkout crawler/walgreens
git rebase develop_hy_langgraph
```

### 六、高级使用

#### 1. 切换分支写完代码后如果主分支有变化，怎么保证是基于主分支的

> 将该分支的提交接在某分支之后

1. **确保你在 `develop_test_brightdata` 分支上**：

   首先，确保你当前在 `develop_test_brightdata` 分支上：

   ```
   git checkout develop_test_brightdata
   ```

2. **拉取最新的 `develop` 分支更新**：

   确保本地的 `develop` 分支是最新的，先切换到 `develop` 分支并拉取最新的远程更新：
...
3. **切换回 `develop_test_brightdata` 分支**：

   拉取完 `develop` 分支的更新后，切换回你的功能分支 `develop_test_brightdata`：
...
4. **执行变基操作**：
   ...
   使用 `git rebase` 命令将 `develop_test_brightdata` 分支的提交基于最新的 `develop` 分支：

   ```
   git rebase develop
   ```

   这条命令的作用是将 `develop_test_brightdata` 分支上的提交移到 `develop` 分支的最新提交之后。

5. **解决冲突（如果有）**：

   如果在变基过程中出现了冲突，Git 会暂停并提示你解决冲突。你可以使用以下命令查看哪些文件有冲突：

   ```
   git status
   ```

   手动解决冲突，之后使用以下命令标记已解决的文件：
   ...
   如果在解决冲突时遇到问题，也可以使用以下命令取消变基操作并恢复到变基前的状态：
...
6. **完成变基**：
   ...
   完成变基操作后，你的 `develop_test_brightdata` 分支将基于 `develop` 分支的最新提交。

7. **推送变基后的分支（强制推送）**：

   由于变基会改变提交历史，因此你需要使用 `--force` 强制推送变基后的分支到远程仓库：

   ```
   git push origin develop_test_brightdata --force
   ```
...
首先，确保你在需要撤回提交的分支上。比如，你要撤回的提交在 `develop_test_brightdata` 分支上，那么你需要首先切换到该分支：
...
使用 `git revert` 命令撤回特定的提交。指定你想撤回的提交哈希 `67b5b31df25edb8047da5886ce7674ed7992269e`：

```
git revert 67b5b31df25edb8047da5886ce7674ed7992269e
```

这会创建一个新的提交，撤销该提交所做的更改。

**解决可能的冲突**：

如果撤回的提交和后续提交之间有冲突，Git 会提示你解决冲突。你可以使用 `git status` 查看冲突文件，并手动编辑解决冲突。
...
当你完成撤回操作后，你可以将新的提交推送到远程仓库：

```
git push origin develop_test_brightdata
```
...
```bash
curl -X POST http://31.97.215.145:8090/ocr/submit-async \
    -H "Content-Type: application/json" \
    -d '{
        "id": "692991af3237e8cc633612d5",
        "image_urls": [
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dw57609464/hi-res/493949_GNC_Womens_50+_Vitapak_Hero.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dwb3a9263c/hi-res/493949_GNC_Womens_50+_Vitapak_Top.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dw1684404e/hi-res/493949_EBC_Thumbnails_02_GNC_Womens_50Plus_Vitapak_TikTok_FactsPanel_Multi_Fish.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dwec548b4d/hi-res/493949_EBC_Thumbnails_03_GNC_Womens_50Plus_Vitapak_TikTok_FactsPanel_CoQ10.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dw2a79c87f/hi-res/493949_EBC_Thumbnails_08_GNC_Womens_50Plus_Vitapak_TikTok_5in1_Save.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dwf032804f/hi-res/493949_EBC_Thumbnails_05_GNC_Womens_50Plus_Vitapak_TikTok_Benefits.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dw59cc5103/hi-res/493949_EBC_Thumbnails_04_GNC_Womens_50Plus_Vitapak_TikTok_WellnessOnePack.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dw3f89a09e/hi-res/493949_EBC_Thumbnails_06_GNC_Womens_50Plus_Vitapak_TikTok_Pills.jpg?sw=1500&sh=1500&sm=fit",
            "https://www.gnc.com/dw/image/v2/BBLB_PRD/on/demandware.static/-/Sites-master-catalog-gnc/default/dw4f437139/hi-res/493949_EBC_Thumbnails_07_GNC_Womens_50Plus_Vitapak_TikTok_Standards.jpg?sw=1500&sh=1500&sm=fit"
        ],
        "prompt": "请识别图片中的所有文字内容，包括产品名称、描述、规格、成分等信息。"
    }'
```

#### 11. tmux 远程连接常用

当我们通过 ssh 远程连接服务器的时候，如果遇到断网就会导致进程终止，可以通过tmux使得进程在服务器后台运行

**1）创建并进入一个 tmux 会话**

在服务器上（VS Code 远程终端/SSH 都行）执行：

```
tmux new -s train
```

你会进入一个新的 tmux 窗口（看起来像一个新的终端）。
...
这叫 **detach**，会回到普通终端，但 tmux 里的任务仍在服务器后台运行。
...
或直接 `Ctrl + d` 退出当前 shell。

A）让程序在 tmux 里跑但输出可回看（写日志）
...
B）tmux 会话被占用（你重复 attach 报错）怎么办

用这个“强制接管”：

```
tmux attach -t train -d
```
...
#### 12. 赋予用户权限
 ...
 `sudo chown -R 50000:0 /opt/app/webscraper/crawler/product`

- `sudo`：用管理员权限执行（因为改属主通常需要 root 权限）
- `chown`：change owner，改“属主/属组”
- `-R`：递归（这个目录下面所有子目录/文件都一起改）
- `50000:0`：把属主改成 **UID=50000**，属组改成 **GID=0**
  - `50000` 通常是 Airflow 容器里 `airflow` 用户的 UID（但要以你 `id` 输出为准）
  - `0` 是 root 组（group id 0）
- `/opt/app/webscraper/crawler/product`：目标目录

效果：让这个目录树里的东西 “属于” UID 50000 的用户，并归到 root 这个组。

 `sudo chmod -R g+rwX /opt/app/webscraper/crawler/product`

- `chmod`：change mode，改“权限位”
- `-R`：递归
- `g+rwX`：给 **组(group)** 增加权限：
  - `g` = group（属组）
  - `+` = 增加（不影响已有权限）
  - `r` = 读
  - `w` = 写
  - `X` = “条件执行”：
    - 如果是目录，就给执行权限（目录的执行权限=允许进入/列出）
    - 如果是文件，只在它本来就有执行权限时才保留（不会把普通文件莫名变成可执行）
...
#### 1. **在 Dockerfile 中配置 UID**

在 Dockerfile 中，你可以使用 `USER` 指令来设置容器中的用户。如果你想设置特定的 UID 和 GID，可以在 Dockerfile 中创建一个用户并指定它的 UID 和 GID。

例如，以下 Dockerfile 创建了一个 UID 为 50000，GID 为 50000 的用户：

```bash
FROM ubuntu:20.04

# 创建一个特定的 UID 和 GID 的用户
RUN groupadd -g 50000 mygroup && useradd -u 50000 -g mygroup myuser

# 切换到该用户
USER myuser
```

在这个例子中，`groupadd` 和 `useradd` 命令会创建一个名为 `myuser` 的用户，UID 为 50000，GID 为 50000。然后使用 `USER` 指令让容器以这个用户身份运行。

#### 2. **在 `docker run` 命令中设置 UID 和 GID**

在运行容器时，可以通过 `--user` 参数指定容器运行时的 UID 和 GID。例如，以下命令将容器的用户设置为 UID 50000 和 GID 50000：

```bash
docker run --user 50000:50000 -it <image_name> bash
```

这样，容器内的进程将以 UID 为 50000 和 GID 为 50000 的身份运行。

#### 3. **在 Docker Compose 中配置 UID**

如果你使用 Docker Compose 启动容器，也可以通过环境变量或 `user` 字段来指定 UID。假设你要设置一个容器的 UID 和 GID，可以在 `docker-compose.yml` 文件中使用 `user` 选项来配置它：
...
services:
  myservice:
    image: <image_name>
    user: "50000:50000"  # 设置容器运行的 UID 和 GID
    volumes:
      - ./data:/data
```

在这个例子中，`user: "50000:50000"` 会确保容器以 UID 为 50000 和 GID 为 50000 的用户身份运行。

#### 4. **通过环境变量配置 UID**

在某些应用程序中，可以通过环境变量动态设置 UID。比如，Airflow 就使用了一个名为 `AIRFLOW_UID` 的环境变量来设置 UID。你可以在 Dockerfile 或 Docker Compose 文件中传递环境变量来配置 UID：
...
services:
  airflow:
    ...
    environment:
      - AIRFLOW_UID=50000  # 设置 UID
    user: "${AIRFLOW_UID}:0"  # 使用环境变量中的 UID
```

这样，容器会使用 `AIRFLOW_UID` 环境变量中的 UID 作为运行时的 UID。

#### 5. **动态修改文件权限**

如果你已经启动了容器并希望更改容器中文件的 UID 和 GID，可以通过 `docker exec` 命令在容器中运行 `chown` 或 `chmod` 来修改文件权限。例如：

```bash
docker exec -u root <container_id> chown -R 50000:50000 /path/to/directory
```

这会将容器内 `/path/to/directory` 目录的所有权更改为 UID 为 50000 和 GID 为 50000 的用户。
