...
通过线程 id 来记录下来历史信息，从而能够实现断点续传机制，每一步保存一次
...
回放实现，通过线程id和checkpoint id一起来实现回放
...
可以通过制定 user_id 来实现跨 session 的对话历史保存

#### 6. Time travel

我们可以编辑修改 state 来实现更新
...
通过数据库实现短期记忆存储，短期记忆通过 thread_id 和 checkpoints 实现
...
当使用 langgraph 开发中间要检查一下的时候，可以用这个方法实现 human_in_loop
...
# 2. 定义中断节点 (approval_node)
def approval_node(state: ApprovalState) -> Command[Literal["proceed", "cancel"]]:
    """
    暂停执行，请求外部（人类）对 action_details 进行审批。
    """
    # 触发中断，并将需要审批的细节作为 payload 返回给调用者
    decision = interrupt({
        "question": "Approve this action?",
        "details": state["action_details"],
    })

    # 当执行恢复时，decision 的值就是 Command(resume=...) 传入的值 (True/False)
    # 根据人类的决定，返回一个 Command(goto=...) 来决定路由
    if decision:
        return Command(goto="proceed")  # 批准，继续执行下一步
    else:
        return Command(goto="cancel")   # 拒绝，跳转到取消节点
...
# approval_node 使用 Command(goto=...) 实现了条件路由
builder.add_edge("proceed", END)
builder.add_edge("cancel", END)

# 编译图，必须指定 Checkpointer 来启用持久性和中断能力
...
# 检查中断返回的 payload
print(f"中断状态: {result.get('__interrupt__')}")
# 预期输出: [Interrupt(value={'question': 'Approve this action?', 'details': 'Transfer $500'})]
print(f"当前状态: {graph.get_state(config).values}")


from langgraph.types import Command

# 传入 Command(resume=True) 来批准，并使用相同的 thread_id
print("\n--- 2. 恢复执行：批准 ---")
...
# 检查最终状态
print(f"最终状态: {resumed_result['status']}")
# 预期输出: approved
```
...
```
--- 1. 首次调用，流程暂停 ---
中断状态: [Interrupt(value={'question': 'Approve this action?', 'details': 'Transfer $500'}, id='5661d7b2f2cc0477e62c3160a81351d5')]
当前状态: {'action_details': 'Transfer $500', 'status': 'pending'}
...
通过 $AgentDefination$ 定义 $subagent$ 
...
class HelloAgentLLm:
    def __init__(self, model: str = None, apiKey: str = None, baseUrl: str = None, provider: str = None, timeout: int = None):
    ...
    def think(self, messages: List[Dict[str, Any]], temperature: float = 0.0) -> str:
        try:
            ...
            print(f"调用LLM API 出现错误: {e}")
            return None


if __name__ == "__main__":
    agent = HelloAgentLLm()
    agent.think([{"role": "user", "content": "你好，世界！"}])
```

如果有多种提供商，比如 openai，modelscope，vllm，可以在 init 中调用下面的逻辑，然后就可以简单初始化 `llm = HelloAgentLLM()` 不需要传入 provider，自动判断
...
设计配置管理，对模型，提供商，温度，最大token数，日志等进行配置，主要是agent常见配置
...
设计agent基类，核心参数就是使用的基座大模型，历史记忆信息，系统提示词（指示该agent的工作和角色），配置信息
...
class Agent(ABC):
    ...
    def __init__(self, name: str, llm: str, system_prompt: Optional[str] = None, config: Optional[Config] = None):
        self.name = name
        self.llm = llm
...
设计一个简单的智能体，首先是 `__init__` 方法加入工具的使用，包括工具注册和是否允许 tool_calling

```python
def __init__(self, name: str, llm: str, system_prompt: Optional[str] = None, config: Optional[Config] = None, tool_registry: Optional['ToolRegistry'] = None,
...
同时需要优化提示词，加入工具的描述信息，告诉大模型有哪些工具可以使用，怎么调用
...
设计 tool_calling，首先是从 response 中获取输出，因为我们在提示词设计是如果进行工具调用返回的格式是 `[TOOL_CALL:{tool_name}:{parameters}]` ，所以要先解析，然后得到工具名和参数后调用，将工具结果添加到messages里，然后得到最终恢复

```python
def _run_with_tools(self, messages: List[Dict[str, Any]], input_text: str, max_tool_iterations: int = 3, **kwargs) -> str:
        """支持多轮工具调用的逻辑"""
        current_iteration = 0
        ...
        if current_iteration >= max_tool_iterations and final_response:
...
我们可以发现上述的工具都是我们自己写的，并在 prompt 中指明可以调用的工具，OpenAI 内部提供了一种封装，就是 function_call，可以参考第五章部分，就是通过构造 tool_shema，然后给大模型描述可以使用的工具信息，就可以直接调用，有特殊的返回格式等
...
将多个工具注册到链表里面，然后执行的时候按照步骤一步一步执行，包括工具名，输入模板和输出结果，输出结果用于下次工具调用的输入
...
![image-20251112151306410](assets/image-20251112151306410.png)
